...
obs_covariates_long = pd.wide_to_long(observational_covariates, stubnames, i='site', j='observation')
obs_covariates_long = obs_covariates_long.rename(columns={"time_observations_started.": "daytime",
                                    "duration_minutes.": "duration",
                                    "effort_distance_km.": "distance_km",
                                    "number_observers.": "n_observers",
                                    "day_of_year.": "day_of_year",
                                    "protocol_type.": "protocol_type"})
obs_covariates_long['protocol_type'] = preprocessing.LabelEncoder().fit_transform(obs_covariates_long['protocol_type'])
...
site_covariates_long = site_covariates.loc[np.tile(np.arange(0, len(site_covariates)), 10)]



## join covariates to one table
covs = np.concatenate((site_covariates_long.values, obs_covariates_transformed), axis=1)
column_names = np.append(site_covariates_long.columns, obs_covariates_long.columns)
...
for i in range(n_sim):
    ...
    for name in column_names:
        ...
        dummy_data = np.tile(mean_x[:, None], 50)  # np.zeros((len(column_names), 50))
...
plt.yticks(np.arange(len(importance[filter_index])), selected_columns[np.argsort(importance[filter_index])], size=12)
...
for i, name in enumerate(selected_columns[np.argsort(importance[filter_index])][::-1]):
    if i < 4:
        plt.subplot(2,2, i+1)
